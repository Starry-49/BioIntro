...
def build_parser() -> argparse.ArgumentParser:
    ...
    extract_parser = subparsers.add_parser("extract", help="Normalize a source file into KB JSON")
    extract_parser.add_argument("--source", required=True)
    extract_parser.add_argument("--out", default=str(ROOT / "example" / "data" / "knowledge-base" / "synquest-extracted.json"))
    extract_parser.set_defaults(func=cmd_extract)

    synth_parser = subparsers.add_parser("synthesize", help="Generate question JSON from KB")
    synth_parser.add_argument("--kb", required=True)
    synth_parser.add_argument("--style-bank", help="Existing question-bank JSON for style retrieval")
    synth_parser.add_argument("--style-top-k", type=int, default=5)
    synth_parser.add_argument("--semantic-model", help="sentence-transformers model name")
    synth_parser.add_argument("--count", type=int, default=12)
    ...
    merge_parser = subparsers.add_parser("merge", help="Merge generated questions into main bank")
    merge_parser.add_argument("--bank", required=True)
    merge_parser.add_argument("--incoming", required=True)
    merge_parser.add_argument("--out", help="Output path; defaults to overwriting --bank")
