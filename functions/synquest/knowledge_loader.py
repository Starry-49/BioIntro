...
DEFAULT_STOPWORDS = {
    "一个", "一些", "一种", "不是", "主要", "以及", "以上", "以下",
    "内容", "包括", "包含", "可以", "这个", "这里", "相关", "通过",
    "进行", "用于", "需要", "说明", "作用", "研究", "方法", "问题",
    "分析", "数据", "课程", "课时",
    "example", "portal", "slide", "slides",
...
QUESTION_EXTRACTION_FIELDS = {
    "question", "answer", "explanation", "difficulty", "type", "distractors",
}
...
def _guess_answer_from_line(line: str) -> tuple[str, str]:
    ...
    numeric_match = re.search(
        r"(\d{4}年|\d+(?:\.\d+)?\s*(?:bp|kb|Mb|Gb|cM|cR|%|倍|次|个))", stripped
    )
    if numeric_match:
...
def _extract_pptx_payload(path: Path) -> dict[str, Any]:
    ...
    return {"slides": slides, "algorithms": PPTX_ALGORITHMS}
...
def _normalize_pdf_entries(path: Path) -> tuple[list[dict[str, Any]], dict[str, Any], str]:
    ...
    for page_number in range(1, page_count + 1):
        ...
        if not body and cleaned_layout:
            body = [
                line for line in cleaned_layout
                if _normalize_line_for_hash(line) != _normalize_line_for_hash(title)
            ]
        if not body:
            body = [title]
...
def build_knowledge_base(path: Union[str, Path]) -> dict[str, Any]:
    ...
    if suffix == ".json":
        ...
        return {
            "meta": {"sourcePath": str(source), "sourceType": "json", "algorithms": ["structured_json_passthrough"]},
            "entries": entries,
        }
